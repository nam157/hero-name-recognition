...
from train import (DenseNet121, GameDataset, create_dataset, test_transforms,
                   train, train_transforms, val)
...
if __name__ == "__main__":
    ...
    train_parser.add_argument('-e','--epochs',default=10,help='Numbers loop traning',type=int)
    train_parser.add_argument('-p','--path_dataset',default='./test_data/test.txt',help='Datasets')

    export_parser = subparsers.add_parser("export", help="Export model to TorchScript format")
    export_parser.add_argument('-c','--convert_model',default='./save_model/hero_model.pth',help='Export model to TorchScript format')

    infer_parser = subparsers.add_parser(
        "infer",
        help="Infer data with TorchScript model, export report if label(s) provided",
    )
    infer_parser.add_argument('-m','--torchjit_ck',default='./save_model/model_hero_jit.pt',help='torchscript model')
    infer_parser.add_argument('-f','--folder_img',default='./test_data/test_images',help='Folder Image')
    infer_parser.add_argument('-s','--save_file',default='./output.txt',help='save results')
    ...
    if opts.get('action') == 'infer':
        test(file_convert_model=opts['torchjit_ck'],folder_img=opts['folder_img'],save_file=opts['save_file'])
